...
import { Shield, Users, Check, X, Edit2, Save } from 'lucide-react'
      ...
      <div className="flex gap-2">
        {(['liste', 'permissions'] as const).map(o => (
          <button key={o} onClick={() => setOnglet(o)}
            ...
            {o === 'liste' ? `Utilisateurs (${mockUsers.length})` : 'Matrice des permissions'}
          </button>
        ))}
      </div>
                  ...
                  <tr key={u.id} onClick={() => setSelected(u)}
                    ...
                    <td className="px-4 py-3.5">
                      {can('utilisateurs.edit') && u.id !== currentUser?.id && (
                        <button onClick={e => { e.stopPropagation(); switchUser(u.id) }}
          ...
          <div className="bg-white rounded-xl border border-surface-200 shadow-card p-5">
            {!selected ? (
              ...
              <div className="space-y-4">
                ...
                {can('utilisateurs.edit') && selected.id !== currentUser?.id && (
                  <button onClick={() => switchUser(selected.id)}
                    className="w-full py-2 bg-surface-100 hover:bg-brand-50 text-surface-700 hover:text-brand-700 rounded-lg text-xs font-semibold transition-colors flex items-center justify-center gap-2">
                    <Edit2 size={12} /> Tester ce compte
                  </button>
                )}
              </div>
            )}
